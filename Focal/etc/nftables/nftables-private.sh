...
#
# nftables-private.sh - DevOpsBroker nftables private network firewall script
#
...
#fi
#
#printBox "DevOpsBroker $UBUNTU_RELEASE nftables Private Configurator" 'true'
#
...
#
# ${NIC}-private.nft - DevOpsBroker nftables private network firewall script
#
...
table ip ipv4-firewall {
  ...
  chain ${NIC}-input {
	# Allow incoming IPv4 Subnet packets
	ip saddr ${IPv4_SUBNET} accept
	ip saddr ${IGMP_SUBNET} accept
...
#	tcp dport \$TCP_SERVICE_PORTS accept
  ...
  chain ${NIC}-output {
	# Allow outgoing IPv4 Subnet packets
	ip daddr ${IPv4_SUBNET} accept
	ip daddr ${IGMP_SUBNET} accept
...
table ip6 ipv6-firewall {
  ...
  chain ${NIC}-input {
	# Allow incoming IPv6 Local Subnet packets
	ip6 saddr ${IPv6_SUBNET_LOCAL} accept
...
#	tcp dport \$TCP_SERVICE_PORTS accept
  ...
  chain ${NIC}-output {
	# Allow outgoing IPv6 Local Subnet packets
	ip6 daddr ${IPv6_SUBNET_LOCAL} accept
